...
# DB Subnet Group (restricted to dedicated Database Subnets)
...
resource "aws_db_instance" "main" {
  ...
  storage_type          = "gp3"
  engine                = "mysql"
  engine_version        = "8.0" # Stable engine version
  instance_class        = var.db_instance_class
  db_name               = "ticketing"
  username              = "admin"
  password              = "password123" # In production, this should be fetched from Secrets Manager or injected securely

  db_subnet_group_name   = aws_db_subnet_group.main.name
  vpc_security_group_ids = [aws_security_group.rds.id]

  # Dev vs Prod Hardening Parameters
...
resource "aws_security_group" "rds" {
  ...
  # Allow MySQL connections from EKS workers and Bastion host
  ingress {
    from_port = 3306
    to_port   = 3306
    protocol  = "tcp"
