...
resource "aws_elasticache_replication_group" "main" {
  ...
  # Apply transit encryption if required for security hardening
  transit_encryption_enabled = false # Set true in production if encryption is required
...
resource "aws_security_group" "redis" {
  ...
  # Allow Redis connections from EKS workers and Bastion host
  ingress {
    from_port = 6379
    to_port   = 6379
    protocol  = "tcp"
